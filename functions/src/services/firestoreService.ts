...
function buildCityLevelEventReviewSample(
  ...
  return compactRecord({
    ...
    observedLocationName: input.observedLocationName || input.locationLabel,
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!snapshot.exists) {
      ...
      const payload: CityLevelEventReviewRecord = {
        ...
        observedLocationName: asOptionalTrimmedString(input.observedLocationName),
        autoPublishSource: input.autoPublishSource,
        autoPublishFieldSources: input.autoPublishFieldSources,
        autoPublishReviewReasons: input.autoPublishReviewReasons,
    ...
    tx.set(
      ...
      compactRecord({
        ...
        observedLocationName: asOptionalTrimmedString(existing.observedLocationName) ||
          asOptionalTrimmedString(input.observedLocationName),
        autoPublishSource: existing.autoPublishSource || input.autoPublishSource,
        autoPublishFieldSources: existing.autoPublishFieldSources || input.autoPublishFieldSources,
        autoPublishReviewReasons: Array.from(new Set([
          ...(Array.isArray(existing.autoPublishReviewReasons) ? existing.autoPublishReviewReasons : []),
          ...(Array.isArray(input.autoPublishReviewReasons) ? input.autoPublishReviewReasons : []),
        ])),
        spatialEvidence: existing.spatialEvidence || input.spatialEvidence,
