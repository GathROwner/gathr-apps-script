...
if (!process.argv.includes('--apply-venues')) {
  throw new Error('Refusing to write without --apply-venues');
}
...
async function main() {
  ...
  const actions = Array.isArray(plan.venueActions) ? plan.venueActions : [];
  if (actions.length !== EXPECTED_COUNT) {
    throw new Error(`Plan contains ${actions.length} venue actions; expected ${EXPECTED_COUNT}`);
  }
  ...
  if (stale.length > 0) {
    throw new Error(`Stale-state check failed for ${stale.length} venue(s): ${JSON.stringify(stale)}`);
  }

  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const backupPath = path.resolve(
    __dirname,
    `artifacts/address-normalization-cleanup-backup-${timestamp}.json`
  );
  const backup = {
    createdAt: new Date().toISOString(),
    projectId,
    approvedScope: 'venue_documents_only',
  ...
  const resultPath = path.resolve(
    __dirname,
    `artifacts/address-normalization-cleanup-result-${timestamp}.json`
  );
  const result = {
