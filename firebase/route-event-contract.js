...
const RENDERABLE_APPROXIMATE_SOURCES = new Set([
  'official_streets',
  'routed_streets',
]);
...
function validateRouteData(routeData) {
  ...
  const confirmedStreetNames = new Set(
    (routeData?.confirmedStreets || []).map((street) =>
      String(street).trim().toLocaleLowerCase('en-CA')
    )
  );
  ...
  for (const [index, segment] of (routeData?.segments || []).entries()) {
    ...
    if (!VALID_CERTAINTIES.has(segment.certainty)) {
      errors.push(`segment ${index} certainty is invalid`);
    }
    if (
      !Array.isArray(segment.coordinates) ||
      segment.coordinates.length < 2 ||
      !segment.coordinates.every(isCoordinate)
    ) {
      errors.push(`segment ${index} coordinates are invalid`);
    }
    if (
      segment.certainty === 'approximate' &&
      !RENDERABLE_APPROXIMATE_SOURCES.has(segment.source)
    ) {
      errors.push(
        `segment ${index} would be hidden by the app; approximate segments need official_streets or routed_streets`
      );
    }
    if (segment.certainty === 'confirmed' && segment.source === 'routed_streets') {
      if (routeData?.status !== 'verified') {
        errors.push(`segment ${index} cannot be confirmed from routed streets unless the route is verified`);
      }
      if (routeData?.evidenceLevel !== 'official_full_route') {
        errors.push(`segment ${index} needs official_full_route evidence`);
      }
      if (routeData?.geometryMethod !== 'map_aligned_street_trace') {
        errors.push(`segment ${index} needs map_aligned_street_trace geometry`);
      }
